            ...
            <header className="border-b fixed top-0 left-0 right-0 h-16 flex bg-white z-10">
                <nav
                    ...
                    <div className="lg:flex lg:flex-1 lg:justify-end">
                        {user ? <div><span className="hidden md:inline-block" >Welcome: {user.email}</span> <Button className="mx-3" onClick={goToSharePage}>Share a movie</Button> <Logout /></div> : <LoginAndRegister />}
                    </div>
                </nav>
            </header>
            <NotificationList />
